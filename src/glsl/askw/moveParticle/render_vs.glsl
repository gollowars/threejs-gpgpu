...
void main() {
    ...
    destination = destination + destination*0.2*snoise(vec3(abs(sin(time*0.05))*destination.x,destination.y,destination.z));
    ...
    float tmpSize = pointSize * (150.0 / length(distPosition.xyz));
    float sizeAmount = abs(sin(time/2.0 + color.z)) + 0.5;
    float actSize = tmpSize * sizeAmount;
    gl_PointSize = size = tmpSize;
}